...
class CartProduct(models.Model):
    ...
    def display_name(self):
        model_fields = self.MODEL_CART_PRODUCT_DISPLAY_NAME_MAP.get(
...
class Cart(models.Model):
    ...
    owner = models.ForeignKey('Customer', verbose_name='Покупатель', null=True, blank=True, on_delete=models.CASCADE)
...
class Order(models.Model):
    ...
    created_at = models.DateField(verbose_name='Дата создания заказ', auto_now=True)
...
class NotificationManager(models.Manager):
    ...
    def read_all_notifications(self, recipient):
        ...
        if customers.count():
            for customer in customers:
                Notification.objects.create(
                    recipient=customer,
                    text=mark_safe(
                        f'Товар <a href="{instance.get_absolute_url()}">{instance.name}</a>'
                        f' из Вашего листа ожидания появился в наличии')
                )
                customer.wishlist.remove(instance)


post_save.connect(send_notification, sender=Album)
pre_save.connect(check_qty, sender=Album)
